...
{
	[CustomEditor(typeof(RuleTileAsset))]
	...
	internal class RuleTileEditor : Editor
	{
		...
		public RuleTileAsset tile { get { return (target as RuleTileAsset); } }
		...
		public void OnEnable()
		{
			if (tile.m_TilingRules == null)
                tile.m_TilingRules = new List<RuleTileAsset.TilingRule>();
		...
		{
			RuleTileAsset.TilingRule rule = tile.m_TilingRules[index];
		...
		private static void RuleMatrixOnGUI(Rect rect, RuleTileAsset.TilingRule tilingRule)
		{
			...
			for (int y = 0; y <= 2; y++)
			{
				for (int x = 0; x <= 2; x++)
				{
					...
					if (x != 1 || y != 1)
					{
						...
						{
							case RuleTileAsset.TilingRule.Neighbor.This:
								GUI.DrawTexture(r, arrows[y*3 + x]);
								break;
							case RuleTileAsset.TilingRule.Neighbor.NotThis:
								GUI.DrawTexture(r, arrows[9]);
								break;
						}
						if (Event.current.type == EventType.MouseDown && r.Contains(Event.current.mousePosition))
						{
							tilingRule.m_Neighbors[index] = (RuleTileAsset.TilingRule.Neighbor) (((int)tilingRule.m_Neighbors[index] + 1) % 3);
							GUI.changed = true;
							Event.current.Use();
						}
					...
					else
					{
						...
						{
							case RuleTileAsset.TilingRule.AutoTransform.Rotated:
								GUI.DrawTexture(r, autoTransforms[0]);
								break;
							case RuleTileAsset.TilingRule.AutoTransform.MirrorX:
								GUI.DrawTexture(r, autoTransforms[1]);
								break;
							case RuleTileAsset.TilingRule.AutoTransform.MirrorY:
						...
						if (Event.current.type == EventType.MouseDown && r.Contains(Event.current.mousePosition))
						{
							tilingRule.m_AutoTransform = (RuleTileAsset.TilingRule.AutoTransform)(((int)tilingRule.m_AutoTransform + 1) % 4);
							GUI.changed = true;
							Event.current.Use();
						}
					}
		...
		{
			public RuleTileAsset.TilingRule m_Rule;
			public RuleTileAsset.TilingRule.AutoTransform m_NewValue;

			public MenuItemData(RuleTileAsset.TilingRule mRule, RuleTileAsset.TilingRule.AutoTransform mNewValue)
			{
				this.m_Rule = mRule;
		...
		private void SpriteOnGUI(Rect rect, RuleTileAsset.TilingRule tilingRule)
		{
		...
		private static void RuleInspectorOnGUI(Rect rect, RuleTileAsset.TilingRule tilingRule)
		{
			float y = rect.yMin;
			EditorGUI.BeginChangeCheck();
			GUI.Label(new Rect(rect.xMin, y, k_LabelWidth, k_SingleLineHeight), "Rule");
			tilingRule.m_AutoTransform = (RuleTileAsset.TilingRule.AutoTransform)EditorGUI.EnumPopup(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.m_AutoTransform);
			...
			tilingRule.m_Output = (RuleTileAsset.TilingRule.OutputSprite)EditorGUI.EnumPopup(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.m_Output);
			y += k_SingleLineHeight;

			if (tilingRule.m_Output == RuleTileAsset.TilingRule.OutputSprite.Animation)
			{
			...
			if (tilingRule.m_Output == RuleTileAsset.TilingRule.OutputSprite.Random)
			{
			...
			if (tilingRule.m_Output != RuleTileAsset.TilingRule.OutputSprite.Single)
			{
